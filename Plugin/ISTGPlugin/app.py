...
def get_title(tokenizer, model, input_text, incomplete_title):
    input_ids = tokenizer(str(input_text), return_tensors="pt", max_length=512,
...
def so():
    ...
    print(incomplete_title)
    print(body)

    input_text = ' '.join(incomplete_title.split()[:16]) + " <body> " + ' '.join(body.split()[:256])
    print(incomplete_title, input_text)
    title_list = get_title(gen_tokenizer, gen_model, input_text, incomplete_title)
    time_end = time.time()
    return jsonify({'title_List': title_list, 'time': round(time_end - time_start, 2)})


if __name__ == '__main__':
    gen_model = T5ForConditionalGeneration.from_pretrained("./TC4SETG/model")
    gen_tokenizer = T5Tokenizer.from_pretrained("./TC4SETG/model")
    app.run(port=5000)
