...
def computePerfectPrediction(predictions_file, target_file):
    ...
    for i in range(1, 16):
        ...
        with open(predictions_file, encoding='utf-8') as fread:
...
def levenshteinOverList(predictions_file, references_file):
    p = open(predictions_file, 'r', encoding='utf-8')
    data = p.readlines()

    r = open(references_file, 'r', encoding='utf-8')
...
if __name__ == "__main__":
    lans = ['python', 'java', 'javascript', 'c#', 'html', 'php', 'ruby', 'go']

    for lan in lans:
        pre_path = f'./{lan}/pred_codet5.csv'
        gol_path = f'./{lan}/gold_codet5.csv'
        ...
        for i in [1, 3, 5]:
            ...
            print('[@{} tokens]: Perfect : {}% ({}/{})'.format(i, percentage, numbers, len_data))
            perfect_predictions[i] = {'percentage': percentage, '#perfect': numbers, '#tokens': len_data}

        references_overall = []
        candidates_overall = []

        with open(pre_path, 'r', encoding='utf-8') as fread:
            for item in fread.readlines():
                candidates_overall.append(item.strip().lower())

        with open(gol_path, 'r', encoding='utf-8') as fread:
            for item in fread.readlines():
